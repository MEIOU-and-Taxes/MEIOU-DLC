...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT In ) : PDX_COLOR
		{
			...
			vColor = lerp( lerp( WATER_RAMP[WATER_RAMP_STOP - 1].rgb, float3( 1, 1, 1 ), 0.33f ) * UNDERWATER_COLOR_MULTIPLIER, vColor, smoothTransition );
			
			float4 vFoW = GetFoW( In.vPrePos_Fade.xyz, vFoWColor, FoWDiffuse );
			
			// Grab the shadow term
			float fShadowTerm = GetShadowScaled( SHADOW_WEIGHT_RIVER, In.vScreenCoord, ShadowMap );
			vColor = ComposeSpecular( vColor, specular * ( 1.0f - In.vPrePos_Fade.w ) * vWaterSurface.a * ( 1.0f - vFoW.a ) );
			vColor = CalculateLighting( vColor, vBottomNormal, fShadowTerm );
			
			vColor = ApplyFoW( ApplyDistanceFog( vColor.rgb, In.vPrePos_Fade.xyz ), vFoW );
			return float4( vColor, vBottomAlpha * ( 1.0f - In.vPrePos_Fade.w ) * (1.0f - TI ) );
		}
	]]
