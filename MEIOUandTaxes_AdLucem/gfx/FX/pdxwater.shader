...
PixelShader = 
{
	...
	[[
		...
		float4 main( VS_OUTPUT_WATER Input ) : PDX_COLOR
		{
			...
			if ( WAVES )
			{
				float waveFrequency = 1.0f;  //2.0
				float waveStart = 91.5f;  //88
				float waveBase = tex2D( HeightTexture, Input.uv ).r - waveStart / 255.0f;
				waveBase /= 94.4f / 255.0f - waveStart / 255.0f;  //93.7
				waveBase = saturate( waveBase );
				float waveTime = vTime_HalfPixelOffset.x * vTimeScale * WATER_TIME_SCALE * 15.0f;  //10.0
				float waveNoise = tex2D( FoWDiffuse, Input.pos.xz / 256.0f ).a * 4.0f + tex2D( FoWDiffuse, ( Input.pos.xz - float2( 0.0f, waveTime ) )/ 1.0f ).a / 16.0f;
