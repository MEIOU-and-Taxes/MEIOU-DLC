...
PixelShader = 
{
	...
	[[
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
			...
			float3 normal = normalize( tex2D( HeightNormal,Input.uv2 ).rbg - 0.5f ) * 0.6f;
	...
	[[
		...
		float4 main( VS_OUTPUT_TERRAIN Input ) : PDX_COLOR
		{
		...
		#endif
			{
				...
				colorRange = 0.12f * colorRange + 0.46f; //Medium Opacity
				//float2 vBlend = float2( 0.70f, 0.3f ); // Transparent version
